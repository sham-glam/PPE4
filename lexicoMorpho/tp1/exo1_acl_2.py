...
def count_tokens(corpus, removePunctuation=True, removeStopWords=True):
    ...
    keys_to_delete = []
    for token in list(N.keys()):  # Convert keys() to a list to avoid 'dictionary changed size during iteration' error
        if N[token] < 5:
...
def make_bigrams(corpus, removePunctuation=False, removeStopWords=False):
    ...
    bigrams_to_delete = []
    for bigram in B.keys():  # Convert keys() to a list to avoid 'dictionary changed size during iteration' error
        if B[bigram] < 5:
...
def make_bigrams_log(B, N):
    ...
    for bigram in B.keys():
        ...
        B_log[bigram] = math.log2((freq_bigram/sum(B.values())) / ((freq_mot1/sum(N.values()))* (freq_mot2/sum(N.values()))))
    return B_log
